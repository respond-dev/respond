...
Server side requests begin with a handler callback from Node HTTP, Lambda, or Cloudflare Worker. All phases of the request pipeline are executed for each server side request. Filenames that begin with `client` are ignored on the server side.
